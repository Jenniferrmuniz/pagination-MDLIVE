...
function findObj(sortedList, boundary) {
  ...
  if (sortedList.indexOf(result) < 0) return undefined; 
...
function paginateAndSort(apps, req) {
  ...
  if (incomingRange.by == 'name') sorted = sortByName(apps, incomingRange.order);
  else if (incomingRange.by == 'id') sorted = sortByID(apps, incomingRange.order);
